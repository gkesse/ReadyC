...
#define GDECLARE_LIST(GPREFIX, GDATA) \
        ...
        static void GList_RemoveDataAll_##GPREFIX##_##GDATA(GListO_##GPREFIX##_##GDATA* obj, char* data, GLIST_EQUAL_DATA_##GPREFIX##_##GDATA equal, int isRemove, GLIST_REMOVE_##GPREFIX##_##GDATA onRemove); \
...
#define GDEFINE_LIST(GPREFIX, GDATA) \
        ...
        \
        static void GList_RemoveNode_##GPREFIX##_##GDATA(GListNodeO_##GPREFIX##_##GDATA* node, int isRemove, GLIST_REMOVE_##GPREFIX##_##GDATA onRemove) { \
            GDATA lData = node->m_data; \
            if(isRemove == 1) { \
                if(onRemove == 0) free(lData); \
                else onRemove(lData); \
            } \
            free(node); \
        } \
