...
static void GSocketWindows_Start(char* dataName, int major, int minor) {
	...
	GConsole()->Print("[ GSocketWindows ] Version Testeur...\n");
	WORD wVersionRequested = MAKEWORD(2, 2);



	int wsaerr = WSAStartup(wVersionRequested, &lData);
	int lOk = WSAStartup(MAKEWORD(2, 2), &lData);
	if(lOk != 0) {GConsole()->Print(" [ GSocketWindows ] Error GSocketWindows_Start: %d\n", WSAGetLastError()); exit(0);}
	//lData = lDataMap->GetData(lDataMap, dataName, GSocketWindows_MapEqual);
