...
static void GSocketUnix_Clean(char* socketName);
//===============================================
#if defined(__unix)
...
static void GSocketUnix_Bind(char* socketName, char* addressName) {
	...
	GMapO(GSocketUnix_GCHAR_PTR_GSOCKADDR_IN_PTR)* lAddressMap = lSocketUnix->m_addressMap;
	int* lSocket = lSocketMap->GetData(lSocketMap, socketName, GSocketUnix_MapEqual);
	struct sockaddr_in* lAddress = lAddressMap->GetData(lAddressMap, socketName, GSocketUnix_MapEqual);
	int lSize = sizeof(lAddress);
	int lOk = bind(*lSocket, (struct sockaddr*)lAddress, lSize);
	if(lOk == -1) {GConsole()->Print(" [ GSocketWindows ] Error GSocketUnix_Bind: %d\n", errno); exit(0);}
#endif
}
//===============================================
static void GSocketUnix_Listen(char* socketName, int backlog) {
	...
	if(lOk == -1) {GConsole()->Print(" [ GSocketWindows ] Error GSocketUnix_Listen: %d\n", errno); exit(0);}
#endif
}
//===============================================
static void GSocketUnix_Accept(char* socketName, char* clientName) {
	...
	*lSocket2 = accept(*lSocket, (struct sockaddr*)lAddress2, (socklen_t*)&lSize2);
	if(*lSocket2 == -1) {GConsole()->Print(" [ GSocketWindows ] Error GSocketUnix_Accept: %d\n", errno); exit(0);}
#endif
}
//===============================================
static void GSocketUnix_Connect(char* socketName) {
	...
	if(lOk == -1) {GConsole()->Print(" [ GSocketWindows ] Error GSocketUnix_Connect: %d\n", errno); exit(0);}
#endif
}
//===============================================
static void GSocketUnix_Write(char* socketName, char* message, int size) {
	...
	if(lBytes == -1) {GConsole()->Print(" [ GSocketWindows ] Error GSocketUnix_Write: %d\n", errno); exit(0);}
#endif
}
//===============================================
static void GSocketUnix_Read(char* socketName, char* message, int size) {
	...
	if(lBytes == -1) {GConsole()->Print(" [ GSocketWindows ] Error GSocketUnix_Read: %d\n", errno); exit(0);}
	message[lBytes] = 0;
#endif
}
//===============================================
...
static void GSocketUnix_Close(char* socketName) {
	...
	if(lOk == -1) {GConsole()->Print(" [ GSocketWindows ] Error GSocketUnix_Close: %d\n", errno); exit(0);}
#endif
}
//===============================================
static void GSocketUnix_Clean(char* socketName) {
